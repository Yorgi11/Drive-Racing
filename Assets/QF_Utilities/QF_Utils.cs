...
namespace QF_Tools.QF_Utilities
{
    ...
    public static class QF_Coroutines
    {
        ...
        public static IEnumerator LerpPingHangPongOverTime<T>(
            T start, T end, float pingDuration, float hangDuration, float pongDuration,
            Func<T, T, float, T> lerp,
            Action<T> onUpdate, bool unscaledTime = false)
        {
            if (pingDuration > 0f) yield return LerpOverTime(start, end, pingDuration, lerp, onUpdate, unscaledTime);
            else onUpdate?.Invoke(end);
            if (hangDuration > 0f)
            {
                float t = 0f;
            ...
            if (pongDuration > 0f) yield return LerpOverTime(end, start, pongDuration, lerp, onUpdate, unscaledTime);
            else onUpdate?.Invoke(start);
            onUpdate?.Invoke(start);
        } // ---------- //
        public static IEnumerator LerpPingPongOverTimeRepeat<T>(
            T start, T end, int repeatCount, float duration,
            Func<T, T, float, T> lerp, Action<T> onUpdate, bool unscaledTime = false)
        {
            float half = Mathf.Max(0f, duration * 0.5f);
